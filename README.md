# Minecraft Web V16.6 — Ender Dragon + Advancements + LAN

Minecraft Web is a browser-based, Java-first Minecraft client/runtime built with Three.js and a custom voxel/gameplay stack. The current production build is **0.16.6** and targets responsive desktop and mobile play, including iPhone Safari.

The live runtime is:

`index.html` → `runtime-loader.js` → cached `runtime-bundle.js` generated from the ordered numbered source parts

The V16.6 browser entrypoint, loader and PWA cache keys are on **0.16.6**. The cached runtime bundle is regenerated by CI from the ordered numbered source parts.
