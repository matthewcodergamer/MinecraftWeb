...
async function bootMinecraftFromParts(){
 ...
 try{
  loading?.classList?.add('show','v15Loading');if(loadingText)loadingText.textContent='Loading Minecraft Web…';let done=0;
  const parts=await Promise.all(SOURCE_PARTS.map(async(name)=>{const text=await fetchPart(name);done++;if(loadingText)loadingText.textContent=`Initializing Java 26.1 • Photon • World Engine • Assets ${done}/${SOURCE_PARTS.length}`;if(loadingFill)loadingFill.style.width=`${Math.round(done/SOURCE_PARTS.length*92)}%`;return text;}));
  if(loadingText)loadingText.textContent='Starting Minecraft Web V15.3…';if(loadingFill)loadingFill.style.width='96%';
  const source=`${parts.join('\n\n')}\n//# sourceURL=minecraft-v15-3-java-26-1-runtime.js`;const blob=new Blob([source],{type:'text/javascript'}),url=URL.createObjectURL(blob);
 ...
 }catch(error){console.error('[V15.3 BOOT]',error);if(loadingText)loadingText.textContent=`Engine failed: ${error.message}`;loading?.classList?.add('show');}
}
