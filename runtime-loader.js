...
(function installBootVisualsV154(){
 const old=document.getElementById('mcBootVisualsV151')||document.getElementById('mcBootVisualsV152')||document.getElementById('mcBootVisualsV153')||document.getElementById('mcBootVisualsV154');if(old)old.remove();
 const style=document.createElement('style');style.id='mcBootVisualsV154';style.textContent=`
...
async function bootMinecraftFromParts(){
 ...
 try{
  ...
  const source=`${parts.join('\n\n')}\n//# sourceURL=minecraft-v15-4-java-26-1-runtime.js`;const blob=new Blob([source],{type:'text/javascript'}),url=URL.createObjectURL(blob);
  try{
   await import(url);
 ...
 }catch(error){console.error('[V15.4 BOOT]',error);if(loadingText)loadingText.textContent=`Engine failed: ${error.message}`;loading?.classList?.remove('mcBootGone','mcBootLeaving');loading?.classList?.add('show');}
}
