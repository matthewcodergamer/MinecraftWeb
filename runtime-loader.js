...
(function installBootVisualsV169(){
  ...
  style.textContent=`
    @font-face{font-family:'Minecraft Seven';src:url('./assets/fonts/Minecraft-Seven.woff?v=0.16.9') format('woff');font-style:normal;font-weight:400;font-display:swap}
...
function showBootFailure(error){
  ...
  console.error('[V16.9 BOOT]',error);
...
async function importSourceFallback(){
  ...
  const source=`${parts.join('\n\n')}\n//# sourceURL=minecraft-v16-9-source-fallback.js`;
...
async function bundleIsCurrent(){
  try{
    ...
    return meta?.build===RUNTIME_BUILD_ID&&Array.isArray(meta.parts)&&meta.parts.includes('126-v16-9-bugfix-master.js');
  }catch{return false}
}
...
async function bootMinecraft(){
  ...
  try{
    ...
    else{console.warn('[V16.9 BOOT] cached bundle is stale; using source recovery until GitHub Actions publishes 0.16.9.');await importSourceFallback()}
