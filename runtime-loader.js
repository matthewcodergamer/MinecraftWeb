...
(function installBootVisualsV1592(){
  ...
  style.textContent=`
    @font-face{font-family:'Minecraft Seven';src:url('./assets/fonts/Minecraft-Seven.woff?v=0.16.4.1') format('woff'),url('https://raw.githubusercontent.com/Mojang/web-theme-bootstrap/main/assets/fonts/Minecraft-Seven_v2.woff') format('woff');font-style:normal;font-weight:400;font-display:swap}
...
function showBootFailure(error){
  ...
  console.error('[V16.4.1 BOOT]',error);
...
async function importSourceFallback(){
  ...
  const source=`${parts.join('\n\n')}\n//# sourceURL=minecraft-v16-4-1-source-fallback.js`,blob=new Blob([source],{type:'text/javascript'}),url=URL.createObjectURL(blob);
...
async function bootMinecraft(){
  ...
  try{
    try{await importCachedRuntime()}catch(error){if(!shouldUseSourceFallback(error))throw error;console.warn('[V16.4.1 BOOT] cached bundle unavailable; using bounded source fallback.',error);await importSourceFallback()}
