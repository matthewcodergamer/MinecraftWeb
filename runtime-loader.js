...
function ensureRetryButton(){
  ...
  if(!b){
    ...
    b.id='mcBootRetryV1592';
    b.type='button';
    b.textContent='Retry Minecraft';
...
function dismissBootOverlay(){
  ...
  setTimeout(()=>{
    loading.classList.remove('show','mcBootLeaving','v1591WorldLoading');
...
function revealTitleIfIdle(){
  const title=document.getElementById('titleScreen');
  if(!title)return;
  if(!window.game?.running&&!window.game?.__hardQuitV159){
    title.style.display='';
...
async function fetchPart(name){
  ...
  for(const url of candidates){
    try{
      const response=await fetch(versioned(url),{cache:'force-cache'});
...
async function fetchPartsBounded(){
  const output=new Array(SOURCE_PARTS.length);
  let cursor=0,done=0;
  const worker=async()=>{
    while(true){
      const i=cursor++;
      if(i>=SOURCE_PARTS.length)return;
      output[i]=await fetchPart(SOURCE_PARTS[i]);
      done++;
      setBootStatus(`Recovering Minecraft runtime • ${done}/${SOURCE_PARTS.length}`,10+(done/SOURCE_PARTS.length)*70);
      if((done&3)===0)await new Promise(resolve=>setTimeout(resolve,0));
    }
  };
  const count=Math.min(6,SOURCE_PARTS.length);
  await Promise.all(Array.from({length:count},worker));
  return output;
}

function shouldUseSourceFallback(error){
  const msg=String(error?.message||error||'').toLowerCase();
  return error instanceof TypeError&&(/fetch|network|module script|dynamically imported|load/.test(msg));
}

async function importSourceFallback(){
  const parts=await fetchPartsBounded();
  setBootStatus('Preparing compatibility runtime…',84);
  await nextPaint();
  const source=`${parts.join('\n\n')}\n//# sourceURL=minecraft-v15-9-2-source-fallback.js`;
...
async function importCachedRuntime(){
  ...
  if(!preload){
    ...
    preload.dataset.mcRuntimePreload='1';
    document.head.appendChild(preload);
  }
  setBootStatus('Loading Minecraft runtime…',36);
  await nextPaint();
  setBootStatus('Starting Minecraft…',92);
  await nextPaint();
...
async function bootMinecraft(){
  ensureBootVisible();
  ensureRetryButton()?.classList.remove('show');
  setBootStatus('Loading Minecraft…',4);
  window.__MC_RUNTIME_READY__=false;
  ...
  addEventListener('unhandledrejection',onReject,true);
  try{
    try{
      await importCachedRuntime();
    }catch(error){
      if(!shouldUseSourceFallback(error))throw error;
      console.warn('[V15.9.2 BOOT] cached bundle unavailable; using bounded source fallback.',error);
      await importSourceFallback();
    }
    ...
    showBootFailure(error||window.__MC_LAST_STARTUP_ERROR__);
  }finally{
    removeEventListener('error',onError,true);
    removeEventListener('unhandledrejection',onReject,true);
  }
}
