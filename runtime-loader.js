...
(function installBootVisualsV1611(){
  ...
  style.textContent=`
    ...
    #mcBootRetryV1611{display:none;position:relative;z-index:3;margin-top:14px;min-width:180px;height:40px;padding:0 18px;background:#777;border:2px solid #000;box-shadow:inset 2px 2px #aaa,inset -2px -2px #333;color:#fff;font:14px 'Minecraft Seven','Courier New',monospace;text-shadow:1px 1px #222;cursor:pointer}
    #mcBootRetryV1611.show{display:block}
  `;
...
function ensureRetryButton(){
  ...
  let b=document.getElementById('mcBootRetryV1611');
  if(!b){b=document.createElement('button');b.id='mcBootRetryV1611';b.type='button';b.textContent='Retry Minecraft';b.onclick=()=>location.reload();loading.appendChild(b)}
  return b;
}
function showBootFailure(error){
  const message=String(error?.message||error||'Unknown startup error');
  console.error('[V16.11 BOOT]',error);
...
async function importSourceFallback(){
  ...
  const source=`${parts.join('\n\n')}\n//# sourceURL=minecraft-v16-11-source-fallback.js`;
...
async function bundleIsCurrent(){
  try{
    ...
    return meta?.build===RUNTIME_BUILD_ID&&Array.isArray(meta.parts)&&meta.parts.includes('128-v16-11-world-reveal-spawn-gate.js');
  }catch{return false}
}
...
async function bootMinecraft(){
  ...
  try{
    ...
    else{console.warn('[V16.11 BOOT] cached bundle is stale; using source recovery until GitHub Actions publishes 0.16.11.');await importSourceFallback()}
