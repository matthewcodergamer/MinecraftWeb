...
function dismissBootOverlay(loading){if(!loading)return;loading.classList.add('mcBootLeaving');loading.setAttribute('aria-hidden','true');setTimeout(()=>{loading.classList.remove('show','mcBootLeaving');loading.classList.add('mcBootGone');loading.style.setProperty('display','none','important');loading.style.setProperty('visibility','hidden','important');loading.style.setProperty('pointer-events','none','important');},180);}
async function bootMinecraftFromParts(){
 ...
 try{
  loading?.classList?.remove('mcBootGone','mcBootLeaving');if(loading){loading.style.removeProperty('display');loading.style.removeProperty('visibility');loading.style.removeProperty('pointer-events');loading.removeAttribute('aria-hidden');}
  ...
  const source=`${parts.join('\n\n')}\n//# sourceURL=minecraft-v15-7-java-26-1-runtime.js`;const blob=new Blob([source],{type:'text/javascript'}),url=URL.createObjectURL(blob);
  try{await import(url);window.__MC_RUNTIME_READY__=true;window.__MC_RUNTIME_BUILD_ID__=RUNTIME_BUILD_ID;if(loadingFill)loadingFill.style.width='100%';if(loadingText)loadingText.textContent='Ready';if(window.__MC_BOOT_WATCHDOG__)clearTimeout(window.__MC_BOOT_WATCHDOG__);requestAnimationFrame(()=>requestAnimationFrame(()=>dismissBootOverlay(loading)));}finally{setTimeout(()=>URL.revokeObjectURL(url),1000);}
 }catch(error){console.error('[V15.7 BOOT]',error);if(loadingText)loadingText.textContent=`Engine failed: ${error.message}`;loading?.classList?.remove('mcBootGone','mcBootLeaving');loading?.classList?.add('show');}
}
